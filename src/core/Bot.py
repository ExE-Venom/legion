...
class Quotient(commands.AutoShardedBot):
    def __init__(self, **kwargs: Any) -> None:
        super().__init__(
            ...
            activity=discord.Activity(type=discord.ActivityType.listening, name=",help | RexZ Development"),
            **kwargs,
        ) 
...
#        self.dblpy = dbl.DBLClient(self, self.config.DBL_TOKEN, autopost=True)
    ...
    async def __load_extensions(self):
        for ext in self.config.EXTENSIONS:
            await self.load_extension(f"{ext}") #ext) #await bot.load_extension(f"cogs.{extension}")
    ...
    @property
    def prime_link(self):
        return "https://dash.rexz.ml/premium"
    ...
    async def on_command(self, ctx: Context):
        ...
        await self.db.execute("INSERT INTO user_data (user_id, made_premium) VALUES ($1, '{}') ON CONFLICT DO NOTHING", ctx.author.id)

    async def on_ready(self):
        print(f"[Legion] Logged in as {self.user.name}({self.user.id})")
