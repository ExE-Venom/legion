...
class Utility(Cog, name="utility"):
    ...
    async def autopurge(self, ctx: Context):
        """
        Set delete every new message in a channel after a specific interval.
        """
        await ctx.send_help(ctx.command)
...
#                f"\nHowever [Premium]({ctx.bot.prime_link}) allows you to set autopurge in unlimited channels."
#            )
