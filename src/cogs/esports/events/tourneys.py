...
class TourneyEvents(Cog):
            ...
            if role := tourney.modrole:
                if role in after.roles and not role in before.roles:
                    msg = (
                        ...
                        f"• You can now use all `{self.bot.config.PREFIX}tourney` commands.\n"
    ...
    async def on_guild_channel_update(self, before: discord.TextChannel, after: discord.TextChannel):
        if before.name == after.name or not before.name == f"{self.bot.config.BOT_NAME}-tourney-logs":
            return

        if after.permissions_for(after.guild.me).manage_channels:
            return await after.edit(
                name=f"{self.bot.config.BOT_NAME}-tourney-logs",
        ...
        _e = discord.Embed(
            color=discord.Color.red(),
            description=(
                f"Someone renamed this channel kindly rename it back to `{self.bot.config.BOT_NAME}-tourney-logs`, "
                f"**{self.bot.config.BOT_NAME} Tourneys won't work without it.**"
            ),
        )
        await after.send(
    ...
    async def on_guild_role_update(self, before: discord.Role, after: discord.Role):
        ...
        _e = discord.Embed(
            color=discord.Color.red(),
            description=(
                f"Someone renamed {self.bot.config.BOT_NAME}'s tourney-mod role to {after.mention}, kindly rename it back to `tourney-mod`."
                f"**{self.bot.config.BOT_NAME} Tourneys need the name of this role to be `tourney-mod`**"
            ),
        )

        c = discord.utils.get(after.guild.text_channels, name=f"{self.bot.config.BOT_NAME}-tourney-logs")
        if c:
            await c.send(
                embed=_e, content=getattr(after.guild.owner, "mention")
            )  # there is very less chances of getting attribute error on `owner.mention`
