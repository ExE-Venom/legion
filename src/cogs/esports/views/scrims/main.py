...
class ScrimsMain(ScrimsView):
    ...
    async def initial_embed(self):
        _e = discord.Embed(color=0x00FFB3, title=f"{self.ctx.config.BOT_NAME}'s Smart Scrims Manager", url=self.ctx.config.SERVER_LINK)
        ...
        _e.set_footer(
            text=f"{self.ctx.config.BOT_NAME} Prime allows unlimited scrims.",
