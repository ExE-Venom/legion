...
class ScrimSetup(ScrimsView):
    ...
    def initial_message(self):
        ...
        #d_link = f"https://{self.bot.config.BOT_DOMAIN}/dashboard/{self.ctx.guild.id}/scrims/create"#.format(self.ctx.guild.id)

        _e = discord.Embed(color=0x00FFB3, title="Enter details & Press Save", url=self.bot.config.SERVER_LINK)
        _e.description = f"[Join The Support Server For Help]({self.bot.config.SERVER_LINK})"#[`Scrim Creation is a piece of cake through dashboard, Click Me`]({d_link})\n\n"
        ...
        _e.set_footer(
            text=f"{self.bot.config.FOOTER}", icon_url=self.ctx.guild.me.display_avatar.url #{self.bot.config.BOT_NAME} Premium servers can set custom reactions.", icon_url=self.ctx.guild.me.display_avatar.url
        )
