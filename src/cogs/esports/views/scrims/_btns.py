...
class OpenTime(ScrimsButton):
    ...
    async def callback(self, interaction: Interaction):
        ...
        m = await self.ctx.simple(
            ...
            image=f"{self.ctx.config.IMG['TIME']}",
    ...
    async def callback(self, interaction: Interaction):
      ...
      #  if not await self.ctx.is_premium_guild():
       #     return await self.ctx.error(
        #        f"[{self.ctx.config.BOT_NAME} Premium](https://{self.ctx.config.BOT_DOMAIN}/premium) is required to use this feature.", 4
         #   )
        ...
        e.set_image(url=f"{self.ctx.config.IMG['TICKCROSS']}")
...
class SaveScrim(ScrimsButton):
    ...
    async def callback(self, interaction: Interaction):
        ...
        print(self.view.record.open_time)
