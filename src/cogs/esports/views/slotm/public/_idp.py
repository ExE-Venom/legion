...
#                if not await self.view.bot.is_premium_guild(
#                                interaction.guild_id):
#                        return await interaction.followup.send(
#                                "IDP Transfer feature is only available for premium servers.\n\n"
#                                f"*This server needs to purchase [BOT Premium]({self.view.bot.prime_link}) to use this feature.*",
#                                ephemeral=True,
#                        )

                if not (slots := await self.view.record.user_slots(
                                interaction.user.id)):
                        return await interaction.followup.send(
                                "You don't have any slot that can be transferred.",
                                ephemeral=True)

                transfer_view = BaseSelector(interaction.user.id,
                                             SlotSelector,
                                             bot=self.view.bot,
                                             records=slots)
                await interaction.followup.send(
                        "Choose a slot to transfer ID-Pass Role to your teammates:",
                        view=transfer_view,
                        ephemeral=True)
                ...
                users = [
                        member
                        async for member in self.view.bot.resolve_member_ids(
                                interaction.guild, _slot.members)
                ]
                if not users:
                        return await interaction.followup.send(
                                "All your teammates seems to have left the server.",
                                ephemeral=True)
                ...
                if len(users) == 1:
                        ...
                        users_view = BaseSelector(interaction.user.id,
                                                  UserSelector,
                                                  users=users)
                        await interaction.followup.send(
                                "Please select your teammate to transfer ID-Pass Role.",
                                view=users_view,
                                ephemeral=True)
                ...
                self.view.bot.loop.create_task(
                        interaction.user.remove_roles(
                                discord.Object(scrim.role_id)))
                self.view.bot.loop.create_task(
                        interaction.guild.get_member(user_id).add_roles(
                                discord.Object(scrim.role_id)))
                return await interaction.followup.send(
                        f"{emote.check} | ID-Pass Role & Slot ownership transferred to <@{user_id}>",
                        ephemeral=True)
...
class SlotSelector(discord.ui.Select):
        ...
        def __init__(self, bot, records):
                ...
                for record in records[:25]:
                        reg_channel = bot.get_channel(
                                record["registration_channel_id"])
                        _options.append(
                                discord.SelectOption(
                                        label=
                                        f"Slot {record['num']} ─ #{getattr(reg_channel,'name','deleted-channel')}",
                                        description=
                                        f"{record['team_name']} (ID: {record['scrim_id']})",
                                        value=
                                        f"{record['scrim_id']}:{record['assigned_slot_id']}",
                                        emoji="📇",
                                ))

                super().__init__(
                        placeholder="Select slot from this dropdown...",
                        options=_options,
                        max_values=1)
...
class UserSelector(discord.ui.Select):
        ...
        def __init__(self, users: T.List[discord.Member]):
                ...
                super().__init__(
                        placeholder="Select your teammate from this dropdown",
                        options=_options)
