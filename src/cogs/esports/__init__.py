...
class ScrimManager(Cog, name="Esports"):
    ...
    async def smanager(self, ctx: Context):
        """
        Contains commands related to scrims manager.
        """
    ...
    async def set_eztag(self, ctx: Context, *, channel: QuoTextChannel):
        ...
        #    return await ctx.error(
       #         f"Upgrade your server to Premium to setup more than 1 EasyTag channel.\n[Click Me to Purchase]({self.bot.prime_link})"
        #    )
        ...
        role = discord.utils.get(ctx.guild.roles, name=f"{ctx.config.BOT_NAME}-tag-ignore")
        if not role:
            role = await ctx.guild.create_role(
                name=f"{ctx.config.BOT_NAME}-tag-ignore", color=self.bot.color, reason=f"Created by {ctx.author}"
            )
        ...
        embed.description = f"""
        Unable to mention teammates while registering for scrims or tournaments?

        Use `teammate's ID`, `@teammate_name` or `@teammate's_discord_tag` in your registration format. {ctx.config.BOT_NAME} will convert that into an actual discord tag.        
        """
        embed.set_image(url=f"{ctx.config.IMG['EZGIF']}")
...
#        if count == 1 and not guild.is_premium:
 #           return await ctx.error(
  #              f"Upgrade your server to Premium to setup more than 1 Tagcheck channel.\n[Click Me to Purchase]({self.bot.prime_link})"
   #         )
    ...
    async def ssverify(self, ctx: Context):
        ...
        """
         ...
         #   if not ctx.guild.member_count > 20: #and not ctx.guild.id == 779229001986080779:
          #      return await ctx.error("Your server must have atleast 20 members to setup ssverification.")
