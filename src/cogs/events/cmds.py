...
class CmdEvents(Cog):
    ...
    async def bot_check(self, ctx: Context):
        ...
        if self.bot.lockdown is True:
            t = (
                f"**{self.bot.config.BOT_NAME} is getting new features** 🥳\n"
                f"Dear user, {self.bot.config.BOT_NAME} is updating and is not accepting any commands.\n"
                "It will back within **2 minutes**.\n"
            )

            if self.bot.lockdown_msg:
                t += f"\n\n**Message from developer:**\n{self.bot.lockdown_msg} ~ Venom"
        ...
        with suppress(discord.HTTPException):
            ...
            if not member.bot and record.humans:
                for role in record.humans:
                    try:
                        await member.add_roles(discord.Object(id=role), reason="autorole")
