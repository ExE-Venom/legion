...
import psutil
import pygit2
from cogs.misc.helper import format_relative
...
from .dev import *
from .alerts import *
from .views import SetupButtonView, VoteButton#,MoneyButton
...
#        source_url = "https://discord.gg/rexzop"
#
#        if search is None:
...
#        await ctx.send(final_url)
    ...
    async def make_private_channel(self, ctx: Context) -> discord.TextChannel:
     ...
     #   source = f"[Source]({ctx.config.REPOSITORY})"
        ...
        channel = await guild.create_text_channel(
            f"{ctx.config.BOT_NAME}-private", overwrites=overwrites, reason=f"Made by {str(ctx.author)}"
        )
        ...
        e.add_field(
            ...
            inline=False,
            value=f"This channel is made for {ctx.config.BOT_NAME} to send important announcements and activities that need your attention. If anything goes wrong with any of my functionality I will notify you here. Important announcements from the developer will be sent directly here too.\n\nYou can test my commands in this channel if you like. Kindly don't delete it , some of my commands won't work without this channel.",
        )
        e.add_field(
            name="**__Important Links__**", value=f"{support_link} | {invite_link}", inline=False# | {vote_link} | {source}", inline=False
        )
    ...
    async def setup_cmd(self, ctx: Context):
        f"""
        Setup {ctx.config.BOT_NAME} in the current server.
        This creates a private channel in the server. You can rename that if you like.
        {ctx.config.BOT_NAME} requires manage channels and manage wehooks permissions for this to work.
...
#        return "\n".join(self.format_commit(c) for c in commits)
    ...
    async def about(self, ctx: Context):
        f"""Statistics of {ctx.config.BOT_NAME}."""
        ...
        owner = await self.bot.getch(self.bot.get_user, self.bot.fetch_user, 853094854683000894)

        msges = self.bot.seen_messages

        embed = discord.Embed(description="Latest Changes: null")
        embed.title = "Official Support Server"
...
#         e = self.bot.embed(ctx, title="Vote for bot")
#         e.description = (
#             "**Rewards**\n"
