...
async def remind_guild_to_pay(guild: discord.Guild, model: Guild):
    if (_ch := model.private_ch) and _ch.permissions_for(_ch.guild.me).embed_links:
        _e = discord.Embed(
            ...
            title="⚠️__**Pro Ending Soon**__⚠️",
...
async def remind_user_to_pay(user: discord.User, model: User):
    ...
    _e.description = (
        f"This is to remind you that your subscription of **Pro** is ending {discord_timestamp(model.premium_expire_time)}"
        f"\n[*Click Me To Continue Enjoying Pro*](https://quotientbot.xyz/premium)"
    )
