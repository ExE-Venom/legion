...
class PremiumCog(Cog, name="Premium"):
    ...
    async def on_premium_purchase(self, txnId: str):
        ...
        with suppress(discord.HTTPException, AttributeError):
            _e = discord.Embed(
                color=discord.Color.gold(), description=f"Thanks **{member}** for purchasing Premium."
            )
        ...
        _e = discord.Embed(
            color=self.bot.color,
            title="Premium Pro Purchase Successful!",
            url=self.bot.config.SERVER_LINK,
            description=(
                f"{random_greeting()} {member.mention},\n"
                f"Thanks for purchasing Premium. Your server **{upgraded_guild}** has access to Pro features until `{_guild.premium_end_time.strftime('%d-%b-%Y %I:%M %p')}`.\n\n"
                "[Click me to Invite Pro Bot to your server](https://discord.com/oauth2/authorize?client_id=902856923311919104&scope=applications.commands%20bot&permissions=21175985838)\n"
            ),
        )

        if member not in self.bot.server.members:
            _e.description += f"\n\n[To claim your Premium Role, Join]({self.bot.config.SERVER_LINK})."
